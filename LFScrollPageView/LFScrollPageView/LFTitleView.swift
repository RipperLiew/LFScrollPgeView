...
// MARK:- 定义协议
protocol LFTitleViewDelegate : class {
    ///点击选择代理事件
    func titleView(_ titleView : LFTitleView, selectedIndex index: Int)
}

class LFTitleView: UIView {

    // MARK:- 对外属性
    weak var delegate : LFTitleViewDelegate?
    ...
    // MARK:- 计算属性
    fileprivate lazy var nomalColorRGB : (r : CGFloat, g : CGFloat, b : CGFloat) = self.getRGBWithColor(self.style.normalColor)
    fileprivate lazy var selectedColorRGB : (r : CGFloat , g : CGFloat , b : CGFloat) = self.getRGBWithColor(self.style.selectedColor)
    
    // MARK:- 自定义构造函数
    init(frame: CGRect, titles : [String], style : LFTitleStyle) {
...
extension LFTitleView {
    ...
    func setTitleWithProgress(_ progress : CGFloat, sourceIndex : Int , targetIndex : Int) {
        ...
        //2. 颜色的渐变 - 较复杂
        //2.1 取出变化的范围
        let colorDalta = (selectedColorRGB.0 - nomalColorRGB.0, selectedColorRGB.1 - nomalColorRGB.1, selectedColorRGB.2 - nomalColorRGB.2)
        ...
        //2.3 变化targetLabel
        targetLabel.textColor = UIColor(r: nomalColorRGB.0 + colorDalta.0 * progress, g: nomalColorRGB.1 + colorDalta.1 * progress, b: nomalColorRGB.2 + colorDalta.2 * progress)
        
        //3.记录最新的Index
        currentIndex = targetIndex
        
        let moveTotaX = targetLabel.frame.origin.x - sourceLabel.frame.origin.x
        let moveTotaW = targetLabel.frame.width - sourceLabel.frame.width
        
        //4.计算滚动的范围差值
        if style.isShowBottomLine {
            bottomLine.frame.size.width = sourceLabel.frame.width + moveTotaW * progress
            bottomLine.frame.origin.x = sourceLabel.frame.origin.x + moveTotaX * progress
        }
        ...
        //6.计算cover的滚动
        if style.isShowCover {
            coverView.frame.size.width = style.isScrollEnable ? (sourceLabel.frame.width + 2 * style.coverMargin + moveTotaW * progress) : (sourceLabel.frame.width + moveTotaW * progress)
            
            coverView.frame.origin.x = style.isScrollEnable ? (sourceLabel.frame.origin.x - style.coverMargin + moveTotaX * progress) : (sourceLabel.frame.origin.x + moveTotaX * progress)
        }
    }
